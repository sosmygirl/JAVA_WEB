...
``` 
2. web应用启动后web.xml初始化调用applicationcontext-springmvc.xml,通过`context:componet-scan base-package="com.xxx"/>`扫描spring的注解包  
``` 
<!-- 配置注解扫描包 -->
...
public class LoginAction(){
  ...
  /** @RequestMapping(value="/index",method=RequestMethod.GET,params="userid!=123"，headers={"Connection=keep-alive"}) 解释：
  	 * value: 请求路径，默认不写
  	 * method：用于指定请求的方法
  	 * params：指定参数规则
  	 ...
  	 * 
  	 */
  @RequestMapping(value="/index",method=RequestMethod.GET,params="userid!=123"，headers={"Connection=keep-alive"})
  public String index() {
  	 return "index";
  }	
